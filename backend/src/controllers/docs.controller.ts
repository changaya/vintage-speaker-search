 ...
 * GET /api/docs/:id/siblings
 *
 * Returns documents in the same catalog/folder, sorted by filePath (natural page order).
 */
export const getDocumentSiblings = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const documentId = parseInt(id, 10);
    ...
    const document = await prisma.document.findUnique({
      where: { id: documentId },
      select: { id: true, brand: true, category: true, subCategory: true, year: true },
    });
    ...
    // Find siblings: same brand + category + subCategory + year
    // IMPORTANT: Use null explicitly for nullable fields (undefined skips the filter in Prisma)
    const siblings = await prisma.document.findMany({
      where: {
        brand: document.brand,
        category: document.category,
        subCategory: document.subCategory === null ? null : document.subCategory,
        year: document.year === null ? null : document.year,
      },
    ...
    const currentIndex = siblings.findIndex((s) => s.id === documentId);

    sendSuccess(res, {
      current: { id: documentId, index: currentIndex },
      siblings,
 ...
 *
 * Returns categories that have documents, with their counts.
 */
export const getDocumentCategories = async (req: Request, res: Response) => {
  try {
    ...
    const where: any = {
      // Exclude hidden documents from category counts
      NOT: { subCategory: { in: ['service_regulations'] } },
    };
    if (brand && typeof brand === 'string') {
      where.brand = brand.toLowerCase();
    }
