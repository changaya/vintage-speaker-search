 ...
 *
 * Controls which fields are visible in Admin UI and API responses.
 * Hidden fields are still stored in database but not shown in UI or API.
 *
 * Usage:
 * - hidden: Fields that will be excluded from UI and API responses
 * - visible: Fields that will be shown (if null, all non-hidden fields are shown)
 *
