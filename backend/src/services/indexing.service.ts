...
// Brand type
type Brand = 'altec' | 'jbl' | 'phasemation';

// Base paths for library directories
// In Docker: /app/altec-library, /app/jbl-library, /app/phasemation-library
// Local: relative to project root
const isDocker = process.env.NODE_ENV === 'development' &&
  (fs.existsSync('/app/altec-library') || fs.existsSync('/app/phasemation-library'));
const BASE_PATH = isDocker ? '/app' : path.resolve(__dirname, '../../../../');
const LIBRARY_PATHS: Record<Brand, string> = {
  altec: path.join(BASE_PATH, 'altec-library/extracted_text'),
  jbl: path.join(BASE_PATH, 'jbl-library/extracted_text'),
  phasemation: path.join(BASE_PATH, 'phasemation-library/extracted_text'),
};

// Product name patterns for each brand
const PRODUCT_PATTERNS: Record<Brand, RegExp[]> = {
  altec: [
  ...
  phasemation: [
    /\b(CA|CM|CS|CT|CC|EA|MA|SA|PP|PS|PT|DG|EPA|HD)[-\s]?\d{1,4}[A-Z]?\b/gi,
    /\bT[-\s]?\d{2,4}\b/gi, // T-1000, T-300
    /\bP[-\s]?\d[A-Z]?\b/gi, // P-1, P-3G
  ],
};
...
export function extractMetadataFromPath(textPath: string): {
  ...
  if (normalizedPath.includes('altec-library')) {
    ...
    brand = 'jbl';
  } else if (normalizedPath.includes('phasemation-library')) {
    brand = 'phasemation';
  } else {
    return null;
  }
 ...
 */
export function extractProductNames(text: string, brand: Brand): string[] {
